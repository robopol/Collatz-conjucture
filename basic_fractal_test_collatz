...
print("""
****************************************************************************************
Basic fractal -Collatz conjecture : 3n-1, 3n+1

Author: Ing. Robert Polak
Contact Info: robopol@robopol.sk
website: https://www.robopol.sk
Purpose:
        Collatz hypothesis test 3n-1, for basic fractal, 
        n=(3^a-2^a)/(3^a-2^b)
        or
        Collatz hypothesis test 3n+1, for basic fractal,
        n=(3^a-2^a)/(2^b-3^a) 
...
while True:
    ...
    if num[0]==1:
        ...
        while a<=max:        
            ...
            if temp>=0.5:
                ...
                n_int=int(n)
            delta=((3**a-2**b)/2**b)*n_int-(3**a-2**a)/2**b
            if delta<0: delta=abs(delta) 
            # integer solution
            if (3**a-2**a)%(3**a-2**b)==0:
                print("Integer solution 3n-1:")
                print("a=",a,"b=",b)
                n_collatz=(3**a-2**a)//(3**a-2**b)
                print("N=",n_collatz)           
            a_list.append(a); min_list.append(delta); delta_list.append([a,delta])
            a+=1
            if a==num[1]+10: print("waiting...")    
        # plot graph
        plt.title(f'3n-1: Δ=((3^a-2^b)/2^b)*n-(3^a-2^a)/2^b for a={num[1]} ... a={num[2]}')
        plt.grid(True)           
    ...
    if num[0]==2:
        ...
        while a<=max:        
            ...
            if delta<0: delta=abs(delta) 
            # integer solution
            if (3**a-2**a)%(2**b-3**a)==0:
                print("Integer solution 3n+1:")
                print("a=",a,"b=",b)
                n_collatz=(3**a-2**a)//(2**b-3**a)
                print("N=",n_collatz)           
            a_list.append(a); min_list.append(delta); delta_list.append([a,delta])
            a+=1
            if a==num[1]+10: print("waiting...")    
